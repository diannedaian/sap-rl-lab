...
class SapAutoBattlerEnv(gym.Env):
    ...
    def __init__(
        ...
        if self.engine.fullpack_rules:
            team_width += 3  # Coconut, Mushroom, Steak.
            shop_width += 3
        ...
        for row, pet in enumerate(state.team):
            ...
            if self.engine.expanded_rules:
                team[row, offset + 8 : offset + 11] = [
                    ...
                    min(pet.ability_uses / (5 if self.engine.fullpack_rules else 3), 1.0),
                ]
            if self.engine.midgame_rules:
                start = offset + 11
            ...
            if self.engine.tier4_rules:
                ...
                team[row, start : start + 2] = [
                    float(pet.perk == "peanut"),
                    float(pet.perk == "bread"),
                ]
            if self.engine.fullpack_rules:
        ...
        for row, item in enumerate(state.shop):
            ...
            if self.engine.expanded_rules and item.kind == "pet" and item.pet is not None:
                ...
                shop[row, start : start + 3] = [
                    ...
                    min(item.pet.ability_uses / (5 if self.engine.fullpack_rules else 3), 1.0),
                ]
                if self.engine.midgame_rules:
                    start += 3
                    pet = item.pet
                ...
                if self.engine.tier4_rules:
                    ...
                    shop[row, start : start + 2] = [
                        float(item.pet.perk == "peanut"),
                        float(item.pet.perk == "bread"),
                    ]
                if self.engine.fullpack_rules:
                    shop[row, -3:] = [
                        float(item.pet.perk == p) for p in ("coconut", "mushroom", "steak")
                    ]
