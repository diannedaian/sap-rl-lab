...
class ActionCodec:
    ...
    def __init__(self, max_team_size: int = 5, max_shop_size: int = 5, team_merges: bool = False):
        self.max_team_size = max_team_size
        self.max_shop_size = max_shop_size
        self.team_merges = team_merges
    ...
    def _build_actions(self) -> Iterator[Action]:
        ...
        # Append, never renumber the historical actions for a given shop size.
        if self.team_merges:
            for source in range(self.max_team_size):
                for target in range(self.max_team_size):
                    if source != target:
                        yield Action(ActionKind.MERGE_TEAM, source, target)
