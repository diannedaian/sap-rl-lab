...
class Event:
    ...
    tie: float = 0.0
    repeat_level: Optional[int] = None
...
class EventRuntime:
    ...
    def battle(self) -> Tuple[BattleOutcome, int]:
        ...
        while self.alive(0) and self.alive(1):
            ...
            self.before_exchange(left, right)
            if health(left) <= 0 or health(right) <= 0:
                continue
            left_damage = self.attack_damage(left)
            right_damage = self.attack_damage(right)
    ...
    def before_exchange(self, left: Pet, right: Pet) -> None:
        pass

    def attack_damage(self, pet: Pet) -> int:
        return attack(pet) + (3 if pet.perk == "meat_bone" else 0)
