...
class PetSnapshot:
    ...
    def from_pet(cls, pet: Pet) -> PetSnapshot:
        return cls(
            ...
            ability_uses=pet.ability_uses,
            copied_ability=pet.copied_ability,
            sell_bonus=pet.sell_bonus,
        )
    ...
    def to_dict(self):
        result = asdict(self)
        if self.copied_ability is None:
            result.pop("copied_ability")
        if not self.sell_bonus:
            result.pop("sell_bonus")
        return result
        ...
        snapshots = [
            {
                ...
                "pets": [pet.to_dict() for pet in snapshot.pets],
            }
...
def build_scripted_league(
    ...
    *,
    ...
    if engine.expanded_rules:
        # Called after end-turn abilities and before combat, including forced
        # battles. Capturing at the policy's END_TURN action misses Snail buffs
        # and misses every battle that the environment initiates automatically.
        def capture_before_battle(turn, rng, current_catalog, current_config):
            league.add(turn, engine.state.team, label=f"{policy_name}-seed-{episode_seed}")
            return default_opponent(turn, rng, current_catalog, current_config)

        engine.opponent_provider = capture_before_battle
    for episode in range(episodes):
        ...
        while not (engine.state.terminated or engine.state.truncated):
            ...
            if action.kind is ActionKind.END_TURN and not engine.expanded_rules:
                league.add(
                    engine.state.turn,
                    engine.state.team,
