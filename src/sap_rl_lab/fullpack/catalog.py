...
def catalog_from_dict(raw: Mapping[str, Any]) -> Catalog:
    ...
    if rules_version not in {1, 2, 3, 4, 5, 6, 7, 8}:
...
def load_catalog_by_id(catalog_id: str) -> Catalog:
    ...
    if catalog_id in {"turtle-v0.46-tier5-v7", "turtle-v0.46-full-v8"}:
        from .content import catalog_data

        return catalog_from_dict(catalog_data(5 if catalog_id.endswith("v7") else 6))
    names = {
