"""Write-once, paired PPO/KL continuation; no tuning, promotion, or retries."""
...
VAL_SEED, TEST_SEED = 33_000_000, 34_000_000
PLAN = ROOT / "docs/KL_BRAKE_PLAN.md"
PARENT = ROOT / "runs/imitation-v1"
HELPERS = (
    "scripts/audit_kl_brake.py",
...
def prepare(output):
    ...
    write_new(
        output / "protocol.json",
        {
            ...
            "validation_records_per_model": 9,
            "validation_seed": VAL_SEED,
            ...
            "primary_endpoint": "fixed-budget final; validation-selected eligible secondary",
            "test_metrics_used_for_selection": False,
            "automatic_recipe_confirmation": False,
            "limitations": "Three retrospectively selected healthy BC-PPO checkpoints; fresh "
...
def freeze(output):
    ...
    for name, payload in p["arms"].items():
        ...
        candidates = {"final": (done["last_model"], records[history[-1]["checkpoint"]["path"]])}
...
def frozen(output):
    ...
    expected = {f"{name}-final" for name in p["arms"]} | {f"initial-seed{s}" for s in SEEDS}
        ...
        try:
            for name in names[start : start + 2]:
                if time.monotonic() >= deadline:
                    raise TimeoutError("Pipeline deadline reached")
                ...
                child = subprocess.Popen(
                    [
                        ...
                        "-u",
                        "-m",
                        "sap_rl_lab.kl_brake_experiment",
                        stage,
                        "--output",
                        str(output),
                        "--name",
            ...
            while any(c.poll() is None for c in children):
                ...
                if time.monotonic() >= deadline:
                    raise TimeoutError("Four-hour pipeline limit; preserve partial evidence")
                time.sleep(1)
...
def main():
    ...
    if args.stage in ("prepare", "pipeline", "freeze", "summarize"):
        ...
        from audit_kl_brake import run
