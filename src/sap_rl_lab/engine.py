...
class AutoBattler:
    ...
    def __init__(
        ...
        self.tier4_rules = self.catalog.rules_version == 6
        self.midgame_rules = self.catalog.rules_version in {5, 6}
        self.expanded_rules = self.catalog.rules_version in {4, 5, 6}
        self.config = config or (
            GameConfig.turtle_tier4()
            if self.tier4_rules
    ...
    def _buy_food(self, shop_index: int, team_index: int) -> None:
        ...
        if self.tier4_rules and food.effect == "buff_future_shop":
            atk, hp = int(food.params["attack"]), int(food.params["health"])
        ...
        if self.expanded_rules:
            ...
            if food.effect == "buff_random_team" and self.midgame_rules:
                ...
                for recipient in targets:
                    runtime.buff(
                        recipient,
                        int(food.params.get("attack", 0)),
                        int(food.params.get("health", 0)),
                    )
...
def resolve_battle(
    ...
    if catalog.rules_version in {4, 5, 6}:
