"""Bounded weight-initialized Tier4 continuation; old frozen source stays unchanged."""
...
ROOT = parent.ROOT
PARENT = ROOT / "runs/tier4-bc-kl-v1"
PRIOR_EVAL = ROOT / "runs/tier4-evaluation-v1"
PLAN = ROOT / "docs/TIER4_LONG_PLAN.md"
STEPS, INTERVAL = 1_048_576, 262_144
TRAIN_SEEDS = (55101, 55201, 55301)
TEST_SEED, MAX_SECONDS = 68_000_000, 7200
...
def prepare(output):
    ...
    if not (PRIOR_EVAL / "evaluation_complete.json").exists():
    ...
    for path in (
        PLAN,
        Path(__file__),
        ROOT / "tests/test_tier4_long.py",
        PRIOR_EVAL / "evaluation_complete.json",
    ...
    for source_seed, training_seed in zip(parent.SEEDS, TRAIN_SEEDS):
        ...
        if (
            model.num_timesteps != parent.CANDIDATE_STEPS
    ...
    write_new(
        output / "protocol.json",
        {
            ...
            "source_timesteps": parent.CANDIDATE_STEPS,
            "new_timesteps_per_model": STEPS,
            "lineage_timesteps_per_model": parent.CANDIDATE_STEPS + STEPS,
            ...
            curves[name].append(
                {
                    "checkpoint": item,
                    "decision": decision,
                    "lineage_timesteps": parent.CANDIDATE_STEPS + item["timesteps"],
                }
            )
            protect(path)
