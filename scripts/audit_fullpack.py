"""Finite full-pack stress gate: deterministic battles and exact reachable episode replays."""
...
from sap_rl_lab.fullpack.engine import make_pet, resolve_battle
from sap_rl_lab.fullpack.env import SapAutoBattlerEnv
from sap_rl_lab.round3 import source_archive


def audit(output, battles=6000, episodes=40, tier=6):
    output.mkdir(parents=True, exist_ok=False)
    sources = source_archive(output)
    catalog = load_catalog_by_id("turtle-v0.46-full-v8" if tier == 6 else "turtle-v0.46-tier5-v7")
    rng = random.Random(81000000)
    ...
    try:
        ...
        for family_index, family in enumerate(
            ("full_stats", "full_summon", "full_tempo", "random")
        ):
            ...
            try:
                for i in range(episodes):
                    seed = 82000000 + family_index * 10000 + i
...
if __name__ == "__main__":
    ...
    audit(args.output, args.battles, args.episodes, args.tier)
